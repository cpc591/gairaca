...
class RadicacionController extends Controller
{
	...
	public function solicitudes_radicacion(){
        ...
        for($i=0;$i<sizeof($respuestas);$i++){
            ...
            if($respuestas[$i]->radicado == null && $sol_user_accione[0]->accione_id == 11){
                ...
                $respuestas[$i]["nombreDependencia"] = $consultaRespuesta['user_create'];
        ...
        join("users","solicitude_user.user_id","=","users.id")
        ->join("solicitudes","solicitudes.id","=","solicitude_user.solicitude_id")
        ->orderBy('solicitude_user.id','DESC')->
		where('solicitudes.id',$lista[$i]->id)->
        select("users.nombre as nombreDependencia")->first();
        $lista[$i]["nombreDependencia"] = $dep['nombreDependencia'];
    ...
    public function guardar_consecutivo(Request $request){
		...
		$cons = Solicitude::where('consecutivo',$request->consecutivo)->first();
        if($cons != null){
			if(date("Y", strtotime($cons->fecha_radicado)) == date("Y",strtotime($request->fecha))){
				return ["success"=>false, "errores"=>["El radico ya se encuentra registrado con otra solicitud."]];
			}            
        }
    ...
    public function guardar_consecutivoRespuesta(Request $request){
		...
		$resp = Respuesta::where('radicado',$request->consecutivo)->first();
        if($resp != null){
			if(date("Y", strtotime($resp->fecha_radicado)) == date("Y",strtotime($request->fecha))){
				return ["success"=>false, "errores"=>["El radico ya se encuentra registrado con otra solicitud."]];
			}            
        }
        ...
        $solicitude_user = Solicitude_User::
            ...
            ->where('solicitude_user.solicitude_id',$solicitud->solicitude_id)
            
            ->select('respuestas.mensaje','solicitude_user.id as solicitude_user_id','solicitude_user.user_id as user_id')->first();
        ...
        //$solicitude_user = Solicitude_User::where('solicitude_id',$id)->where('user_id',39)->first();
           ...
           // $respuesta = Respuesta::where('solicitude_user_id',$solicitude_user->id)->first();
            //return $respuesta;
        $view =  \View::make('Solicitudes.pdf_respuesta', ['respuesta' => $solicitude_user->mensaje,'user_id'=>$solicitude_user->user_id])->render();
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($view);
        $correosAdjuntos = $radicado["correos"];
        if($correosAdjuntos != null){
            for($i=0;$i<sizeof($correosAdjuntos);$i++){
                $enviarCorreoUser = User::where('id',$correosAdjuntos[$i]["id"])->first();
                try{
                    \Mail::send('VistasEmails.email_respuesta',$data, function($message) use ($pdf, $enviarCorreoUser){
                       //remitente
                       $message->from(env('CONTACT_MAIL'), env('CONTACT_NAME'));
                       ...
                       $message->attachData($pdf->stream(), "respuesta.pdf");
	...
	public function rechazarRespuesta(Request $request){
        ...
        $userRespuesta = Solicitude_User::
            has('respuestas')->
            join("respuestas","respuestas.solicitude_user_id","=","solicitude_user.id")
            ->where('solicitude_user.solicitude_id',$request->idSolicitud)
            
            ->select('solicitude_user.user_id as user_id')->first();
        
        $solicitude_user = Solicitude_User::where('solicitude_id',$request->idSolicitud)->where('user_id',$userRespuesta->user_id)->first();
        $solicitude_user_accione = new Solicitude_User_Accione;
