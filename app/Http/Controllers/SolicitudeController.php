...
class SolicitudeController extends Controller
{
    ...
    public function crear_solicitud_estudiante (Request $request){
        ...
        //return $request->all();
        $busquedaAsuntos = User::with('asuntos')->where('id',$request->Dependencia)->first();
        if(sizeof($busquedaAsuntos['asuntos']) > 0){
            $error = [];
            if($request->asunto == null){
                $error["asunto"][sizeof($error)] = "El campo asunto es requerido.";
                return ["success"=>false,"errores"=>$error];
    ...
    public function listado_solicitudes_estudiantes(){
	    ...
	    $lista_asuntos = Asunto::all();
	    $lista_tipos = Tipo::where('id','<>',4)->get();
	    $lista_dependencia = User::with('asuntos')->where('estado',1)->where('id','<>',Auth::user()->id)->whereHas('roles', function ($query) {
            $query->where('role_id',1);
        })->get();
    ...
    public function obtener_solicitud($id){
		...
		if($solicitude_user != null){
            $respuestas = Respuesta::where('solicitude_user_id',$solicitude_user->id)->with('correos')->first();
        }else{
            $respuestas = null;
        }
